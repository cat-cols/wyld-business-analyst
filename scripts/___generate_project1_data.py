...
DB artifacts (Postgres)
- Sales / POS transactions DB export -> loads to <schema>.<pos_table>
- Finance / GL detail DB export     -> loads to <schema>.<gl_table>
...
def pg_connect(dsn: str):
    mod, kind = _try_import_psycopg()
    # both psycopg3 and psycopg2 expose connect()
...
def _normalize_df_for_pg(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Ensure NaN -> None for DB inserts
...
def pg_truncate(con, schema: str, table: str) -> None:
    with con.cursor() as cur:
        cur.execute(f"truncate table {_pg_ident(schema)}.{_pg_ident(table)};")
    con.commit()
...
def main() -> None:
    ...
    ap.add_argument(
        "--pg-add-metadata",
        choices=["1", "0"],
        default=os.getenv("PROJECT1_PG_ADD_METADATA", "1"),
        help="If 1, adds load_id + drop_date/drop_month columns to DB loads",
    )
    ...
    # Optionally truncate tables at the start of the run (if they already exist)
    if args.pg_load_mode == "truncate_then_append":
        # We only truncate if table exists; simplest safe approach: try/except.
        for t in [args.pg_pos_table, args.pg_gl_table]:
            try:
    ...
    for d in day_range:
        ...
        log(
            ...
            file_type="postgres",
            rows=len(dfp),
            notes="loaded to postgres",
            target=f"{args.pg_schema}.{args.pg_pos_table}",
        )
    ...
    for ms in month_range:
        ...
        # finance: gl detail DB load (postgres)
        ...
        log(
            ...
            rows=len(dfg),
            notes="loaded to postgres",
            target=f"{args.pg_schema}.{args.pg_gl_table}",
        )
